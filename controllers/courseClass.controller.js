...
exports.list = async (req, res) => {
  try {
    // 1. Thêm teacher_code vào đây
    const { page = 1, limit = 20, q = '', status, teacher_code } = req.query; 
    
    // 2. Truyền sang service
    const result = await listCourseClasses({ 
      page, 
      limit, 
      q, 
      status, 
      teacher_code // <--- Quan trọng
    });
