const { listCourseSchedules, getCourseSchedule, createCourseSchedule, updateCourseSchedule, deleteCourseSchedule, autoCreateCourseSchedule } = require('../services/courseSchedule.service');
...
exports.autoCreate = async (req, res) => {
  try {
    ...
    const created = await autoCreateCourseSchedule(req.body);
    res.status(201).json({ message: 'CourseSchedule auto-created', course_schedule: created.schedule, summary: created.summary });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
};
