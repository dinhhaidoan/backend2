const { listGroups, getGroup, createGroup, updateGroup, deleteGroup } = require('../services/group.service');
...
exports.list = async (req, res) => {
  try {
    const { course_class_id, page = 1, limit = 20 } = req.query;
    const result = await listGroups({ course_class_id, page, limit });
    res.json(result);
  } catch (err) {
...
exports.getById = async (req, res) => {
  try {
    ...
    if (err.message === 'Group not found') return res.status(404).json({ error: err.message });
...
exports.create = async (req, res) => {
  try {
    ...
    const created = await createGroup(req.body);
    res.status(201).json({ message: 'Group created', group: created });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
};
...
exports.update = async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const updated = await updateGroup(id, req.body);
...
exports.delete = async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const out = await deleteGroup(id);
    res.json(out);
  } catch (err) {
    if (err.message === 'Group not found') return res.status(404).json({ error: err.message });
    res.status(400).json({ error: err.message });
